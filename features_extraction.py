...
def classical_features_extraction(nifti_path, mask_path, list_patients, settings, verbosity=40, save_extraction=True):
    ...
    for i in range(n_patients):
        ...
        mask_arr= majority_vote(m4,m2p5,m40,m_km,mask_arr)
        
        
        #mask_arr = mask_suv4(image_arr, mask_arr)
...
def mask_kmeans(image_arr,mask_arr):
    ...
    if dim_array==3:
        ...
        ret,label,center=cv2.kmeans(Z,3,None,criteria,10,cv2.KMEANS_PP_CENTERS)
...
def majority_vote(m4,m2p5,m40,m_km,mask_arr):
    
     
    dim_array = len(m2p5.shape)
    ch=np.where(m2p5!=0)
    mMajority=np.zeros_like(mask_arr) 
    ...
    if dim_array == 3 : 
        ...
        for i in range(m2p5.shape[3]):
            ...
            if len(np.unique(m4_3d)) > 1 : 
                
                sum_mask= m4_3d + m2p5_3d + m40_3d + m_km_3d
                sum_mask= np.divide(sum_mask, nbMethods)
                mMajority [:,:,:,i]= np.array(sum_mask>thresh, dtype='int')
