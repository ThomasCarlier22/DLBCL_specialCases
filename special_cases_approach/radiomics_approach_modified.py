...
class parameters:
    ...
    def __init__(self, settings_pyradiomics):
        ...
        self.n_loop_survival = 1 # change to 100
...
X_train_test = np.load("./path_to_features")
y_train_test = np.load("./path_to_outcome")
survival_train_test = np.load("./path_to_survival")
features_name = np.load("./path_to_features_names")
list_patients_int = np.load("./path_to_list")
...
n_patients_test, n_features = np.shape(X_test)
list_patients_int = np.load("./path_to_list")
